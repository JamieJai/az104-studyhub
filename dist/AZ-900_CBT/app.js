(() => {
  ...
  const THEME_KEY = "az900cbt.theme";     // 테마만 기기별 저장. 기록은 전부 서버(D1, exam=sc300).
  const EXAM = "az900";
  ...
  function exportProgress() {
    const blob = new Blob([JSON.stringify({ app: "az900-cbt", exportedAt: nowIso(), records: state.records, examRuns: state.examRuns }, null, 2)], { type: "application/json" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `az900-progress-${new Date().toISOString().slice(0, 10)}.json`; a.click(); URL.revokeObjectURL(a.href);
  }
  function importProgress(file) {
    const fr = new FileReader();
    fr.onload = () => { try { const d = JSON.parse(fr.result); if (d.app !== "az900-cbt") throw 0; for (const [k, v] of Object.entries(d.records || {})) state.records[k] = { ...v, updatedAt: nowIso() }; if (Array.isArray(d.examRuns)) state.examRuns = [...state.examRuns, ...d.examRuns].slice(-30); saveState(); updateStats(); toast("기록을 가져왔습니다 · 서버에 저장 중"); } catch { toast("올바른 기록 파일이 아닙니다"); } };
    fr.readAsText(file);
  }
  ...
  function bind() {
    ...
    $("resetProgress").addEventListener("click", async () => {
      if (!confirm("AZ-900 풀이 기록·북마크·모의고사 기록을 서버에서 삭제할까요? (다른 시험 기록은 그대로)")) return;
