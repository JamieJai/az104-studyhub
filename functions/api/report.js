...
export async function onRequestGet({ request, data, env }) {
  ...
  const rows = await env.DB.prepare("SELECT data FROM reports WHERE user_id = ? AND exam = ? ORDER BY at").bind(data.user.uid, exam).all();
  const reports = [];
...
export async function onRequestPost({ request, data, env }) {
  ...
  await env.DB.prepare(
    "INSERT OR REPLACE INTO reports (user_id, exam, source, kind, data, at) VALUES (?, ?, ?, ?, ?, ?)"
  ).bind(data.user.uid, exam, source, entry.kind, JSON.stringify(entry), entry.at).run();
  const { c } = await env.DB.prepare("SELECT COUNT(*) AS c FROM reports WHERE user_id = ? AND exam = ?").bind(data.user.uid, exam).first();
  return json({ ok: true, count: c });
}

export async function onRequestDelete({ request, data, env }) {
  const exam = examOf(request); if (!exam) return badExam();
  await env.DB.prepare("DELETE FROM reports WHERE user_id = ? AND exam = ?").bind(data.user.uid, exam).run();
  return json({ ok: true });
}
