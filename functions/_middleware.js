 ...
 * - /AZ-104_CBT/ 로 페이지 이동(HTML 요청)하는데 세션이 없으면 / (로그인 화면)으로 보낸다.
 * - /api/* (auth 제외)는 세션 없으면 401.
 * - 로그인된 상태로 / 에 오면 CBT로 보낸다.
...
export async function onRequest(context) {
  ...
  const cbtPage = path === "/AZ-104_CBT" || path === "/AZ-104_CBT/" || path === "/AZ-104_CBT/index.html";
  if (!user && cbtPage && isNav) {
  ...
  if (user && (path === "/" || path === "/index.html") && isNav) {
    ...
    const safe = to && to.startsWith("/") && !to.startsWith("//") ? to : "/AZ-104_CBT/";
