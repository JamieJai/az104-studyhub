...
| `dist/AZ-305_CBT/`, `dist/AZ-900_CBT/`, `dist/AI-103_CBT/` | AZ-305 285문항 · AZ-900 470문항 · AI-103 155문항 (영어 원문, 번역 준비 중). `exam=az305|az900|ai103` |
| `tools/build_cbt.js` | 스크랩 JSON → CBT 한 벌 생성 (`node tools/build_cbt.js <examId> <표시명> <json> <images> <dist폴더>`). 앱 본체는 SC-300 판을 틀로 복사하고 시험 id·브랜딩만 바꾼다 |
...
**시리즈 문항 (`data.js` 의 `series`)** — "Solution: … Does this achieve the goal?" 처럼 한 시나리오에 해법 2~4개가 딸린 Yes/No 2지선다 묶음. 원본에 묶음 정보가 없어 AZ-802 는 지문의 시나리오 표시(참고 문구의 목표·legacy skill), SC-300·AZ-104 는 원본 번호 연속 구간으로 만들었다(AZ-802 9묶음 41문항, SC-300 6묶음 26문항, AZ-104 26묶음 81문항). 사례 연구와 마찬가지로 `unitsOf` 가 통째로 묶어 출제하므로 연습·모의고사·실전 어디서도 흩어지지 않는다. 실제 시험처럼 실전 모의고사에서는 1부(단답)에 들어간다.
